...
class MusicBot(discord.Client):
    ...
    async def _check_ignore_non_voice(self, msg):
        ...
        if not vc or vc == msg.author.voice_channel:
            ...
            raise exceptions.PermissionsError(
                ":x: You cannot use this command when not in the voice channel (%s)" % vc.name, expire_in=30)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = ':musical_note: %s your song **%s** is now playing in %s!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = ':musical_note: Now playing in %s: **%s**' % (
    ...
    async def update_now_playing(self, entry=None, is_paused=False):
        ...
        if self.user.bot:
            ...
            if activeplayers > 1:
                game = discord.Game(type=0, name="music on %s servers; use m!help for commands." % activeplayers)
                entry = None
        ...
        if entry:
            ...
            game = discord.Game(type=0, name="m!help: %s" % name)
    ...
    def run(self):
        try:
            ...
            raise exceptions.HelpfulError(
                "Bot cannot login, incorrect credentials.",
                "Check your email, password, or token in the options file.  "
    ...
    async def cmd_help(self, command=None):
        ...
        Shows a list of commands available by the bot.
        If a command is specified, it shows a help message for the specified command.
        """
        ...
        if command:
            ...
            if cmd:
                ...
                return Response("No command found.", delete_after=10)
            ...
            helpmsg += "```"
            helpmsg += ":exclamation: Some commands may not work depending on your permissions."
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        Adds or removes users to a blacklist.
        Blacklisted users are forbidden from using bot commands.
        """

        if not user_mentions:
            raise exceptions.CommandError(":exclamation: No users listed.", expire_in=20)

        if option not in ['+', '-', 'add', 'remove']:
            raise exceptions.CommandError(
                ':exclamation: Invalid option "%s" specified, use +, -, add, or remove.' % option, expire_in=20
            )
        ...
        if option in ['+', 'add']:
            ...
            return Response(
                ':white_check_mark: %s users have been added to the blacklist.' % (len(self.blacklist) - old_len),
                reply=True, delete_after=10
            )

        else:
            if self.blacklist.isdisjoint(user.id for user in user_mentions):
                return Response(':exclamation: None of these users are in the blacklist.', reply=True, delete_after=10)
                ...
                return Response(
                    ':white_check_mark: %s users have been removed from the blacklist.' % (old_len - len(self.blacklist)),
                    reply=True, delete_after=10
                )
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        if self.user.bot:
            ...
            return Response(
                ":exclamation: Bot accounts can not use invitation links!  Click here to invite me: \n{}".format(url),
        ...
        try:
            ...
            raise exceptions.CommandError(':x: Invalid URL provided:\n{}\n'.format(server_link), expire_in=30)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        Adds a song to the playlist.  If a link is not provided, the first
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                ":exclamation: You have reached your enqueued song limit (%s)." % permissions.max_songs, expire_in=30
            )
        ...
        if not info:
            raise exceptions.CommandError(":x: That video cannot be played.", expire_in=30)
        ...
        if info.get('url', '').startswith('ytsearch'):
            ...
            if not info:
                raise exceptions.CommandError(
                    ":x: Error extracting info from search string, youtubedl returned no data.  "
        ...
        if 'entries' in info:
            if not permissions.allow_playlists and ':search' in info['extractor'] and len(info['entries']) > 1:
                raise exceptions.PermissionsError(":x: You are not allowed to request playlists.", expire_in=30)

            num_songs = sum(1 for _ in info['entries'])

            if permissions.max_playlist_length and num_songs > permissions.max_playlist_length:
                raise exceptions.PermissionsError(
                    ":x: Playlist has too many entries (%s > %s)." % (num_songs, permissions.max_playlist_length),
                    expire_in=30
                )

            if permissions.max_songs and player.playlist.count_for_user(author) + num_songs > permissions.max_songs:
                raise exceptions.PermissionsError(
                    ":x: Playlist entries + your already queued songs have reached the limit (%s + %s > %s)." % (
            ...
            if info['extractor'].lower() in ['youtube:playlist', 'soundcloud:set', 'bandcamp:album']:
                try:
                    ...
                    traceback.print_exc()
                    raise exceptions.CommandError(":x: Error queuing playlist:\n%s." % e, expire_in=30)
            ...
            if not listlen - drop_count:
                raise exceptions.CommandError(
                    ":x: No songs were added, all songs were over the max duration (%ss)." % permissions.max_song_length,
                    expire_in=30
                )

            reply_text = ":white_check_mark: Enqueued **%s** songs to be played. Position in queue: %s."
            btext = str(listlen - drop_count)

        else:
            if permissions.max_song_length and info.get('duration', 0) > permissions.max_song_length:
                raise exceptions.PermissionsError(
                    ":x: Song duration exceeds limit (%s > %s)." % (info['duration'], permissions.max_song_length),
                    expire_in=30
                )
            ...
            reply_text = ":white_check_mark: Enqueued **%s** to be played. Position in queue: %s."
            btext = entry.title
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not info:
            raise exceptions.CommandError(":x: That playlist cannot be played.")

        num_songs = sum(1 for _ in info['entries'])
        t0 = time.time()

        busymsg = await self.safe_send_message(
            channel, ":arrows_counterclockwise: Processing %s songs." % num_songs)
        await self.send_typing(channel)
        ...
        if extractor_type == 'youtube:playlist':
            try:
                ...
                traceback.print_exc()
                raise exceptions.CommandError(':x: Error handling playlist %s queuing.' % playlist_url, expire_in=30)
        ...
        if not songs_added:
            basetext = ":x: No songs were added, all songs were over max duration (%ss)." % permissions.max_song_length
            if skipped:
                basetext += "\nAdditionally, the current song was skipped for being too long."

            raise exceptions.CommandError(basetext, expire_in=30)

        return Response(":musical_note: Enqueued {} songs to be played in {} seconds.".format(
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) > permissions.max_songs:
            raise exceptions.PermissionsError(
                ":x: You have reached your playlist item limit (%s)." % permissions.max_songs,
                expire_in=30
            )
        ...
        def argcheck():
            if not leftover_args:
                raise exceptions.CommandError(
                    ":exclamation: Please specify a search query.\n%s" % dedent(
        ...
        try:
            ...
            raise exceptions.CommandError(":exclamation: Please quote your search query properly.", expire_in=30)
        ...
        if leftover_args[0].isdigit():
            ...
            if items_requested > max_items:
                raise exceptions.CommandError(":x: You cannot search for more than %s videos." % max_items)
        ...
        if not info:
            return Response(":x: No videos found.", delete_after=30)
        ...
        for e in info['entries']:
            ...
            if response_message.content.lower().startswith('y'):
                ...
                return Response(":white_check_mark: Alright, coming right up.", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)
    ...
    async def cmd_nowplaying(self, player, channel, server, message):
        """
        Usage:
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = ":arrow_forward: Now playing:\n**{title}** added by **{author}**\n\nProgress: {progress_bar} {progress}\nURL: <{url}>".format(
            ...
            return Response(
                ':exclamation: There are no songs queued! Queue something with {}play.'.format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError(':exclamation: Player is not playing.', expire_in=30)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError(':exclamation: Player is not paused.', expire_in=30)
    ...
    async def cmd_shuffle(self, channel, player):
        ...
        return Response(":ok_hand: Shuffling has finished.", delete_after=15)

    async def cmd_clearqueue(self, player, author):
        """
        Usage:
            {command_prefix}clearqueue

        Clears the playlist.
        """

        player.playlist.clear()
        return Response(':put_litter_in_its_place: The queue has been cleared.', delete_after=20)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        Skips the current song. May require votes to skip a song.
        """

        if player.is_stopped:
            raise exceptions.CommandError(":exclamation: Can not skip! The player is not playing!", expire_in=20)
            ...
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    ...
                    print("Something odd is happening."
                          "You might want to restart the bot if it stops working.")
            else:
        ...
        if skips_remaining <= 0:
            player.skip()
            return Response(
                ':white_check_mark: Your skip for **{}** was acknowledged.'
                '\n :white_check_mark: Most users agreed to skip, the song has been skipped.{}'.format(
    ...
    async def cmd_remove(self, message, player, index=None):
        ...
        Removes a song in the queue. Use {command_prefix}queue to check the number.
        """
		
        if not index:
            raise exceptions.CommandError(":exclamation: No input found. Try again with a position from (command_prefix)queue.", expire_in=20)
		
        try:
            index = int(index)

        except ValueError:
            raise exceptions.CommandError(':x: {} is not a valid number.'.format(index), expire_in=20)
			
        if index <= len(player.playlist.entries):
            if index > 0:
                song_title = player.playlist.entries[index-1].title
                player.playlist.remove_entry((index)-1)
                return Response(':white_check_mark: **' + song_title + '** has been removed.', delete_after=20)

            else:
                raise exceptions.CommandError(":exclamation: You can not remove the current song, or a song in a position that does not exist.", expire_in=20)

        else:
            raise exceptions.CommandError(":exclamation: There is either no queue, or the queue is not that long.", expire_in=20)	
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response(':loud_sound: Current volume: `%s%%`' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        try:
            ...
            raise exceptions.CommandError(':x: {} is not a valid number'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response(':loud_sound: Updated volume from %d to %d' % (old_volume, new_volume), reply=True, delete_after=20)

        else:
            if relative:
                raise exceptions.CommandError(
                    ':x: Unreasonable volume change provided: {}{:+} -> {}%.  Provide a change between {} and {:+}.'.format(
                        old_volume, vol_change, old_volume + vol_change, 1 - old_volume, 100 - old_volume), expire_in=20)
            else:
                raise exceptions.CommandError(
                    ':x: Unreasonable volume provided: {}%. Provide a value between 1 and 100.'.format(new_volume), expire_in=20)
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        Sets the music volume. Accepted values are from 1 to 100.
        Putting + or - before the volume will make the volume change relative to the current volume.
        """

        if not new_volume:
            return Response(':loud_sound: Current volume level is: `%s%%`.' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response(':loud_sound: Changed volume level from %d to %d.' % (old_volume, new_volume), reply=True, delete_after=20)

        else:
            if relative:
                raise exceptions.CommandError(
                    ':x: Unreasonable volume value provided: {}{:+} -> {}%.  Provide a value between {} and {:+}.'.format(
                        old_volume, vol_change, old_volume + vol_change, 1 - old_volume, 100 - old_volume), expire_in=20)
            else:
                raise exceptions.CommandError(
                    ':x: Unreasonable volume value provided: {}%. Provide a value between 1 and 100.'.format(new_volume), expire_in=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        Shows the list of current songs in queue.
        """
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append(":arrow_forward: Now playing:\n**%s** added by **%s** %s\n" % (
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        try:
            ...
            return Response("Enter a number to clear # amount of lines from the bot.", reply=True, delete_after=8)
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response(':put_litter_in_its_place: Cleaned up {} message{}.'.format(len(deleted), 's' * bool(deleted)), delete_after=15)
        ...
        return Response(':put_litter_in_its_place: Cleaned up {} message{}.'.format(deleted, 's' * bool(deleted)), delete_after=15)
    ...
    async def cmd_pldump(self, channel, song_url):
        ...
        try:
            ...
            raise exceptions.CommandError(":x: Could not extract info from input URL\n%s\n." % e, expire_in=25)

        if not info:
            raise exceptions.CommandError(":x: Could not extract info from input URL, no data.", expire_in=25)

        if not info.get('entries', None):

            if info.get('url', None) != info.get('webpage_url', info.get('url', None)):
                raise exceptions.CommandError(":x: This does not seem to be a playlist.", expire_in=25)
            else:
        ...
        if not exfunc:
            raise exceptions.CommandError(":x: Could not extract info from input url, unsupported playlist type.", expire_in=25)
    ...
    async def cmd_idlist(self, server, author, leftover_args, cat='all'):
        """
        Usage:
            {command_prefix}idlist [categories]
        ...
        return Response(":mailbox_with_mail: I just sent you a direct message.", delete_after=20)


    async def cmd_permission(self, author, channel, server, permissions):
        """
        Usage:
    ...
    async def cmd_setname(self, leftover_args, name):
        ...
        return Response(":white_check_mark: Name change has been completed.", delete_after=20)
    ...
    async def cmd_setnick(self, server, channel, leftover_args, nick):
        ...
        if not channel.permissions_for(server.me).change_nickname:
            raise exceptions.CommandError(":x: Unable to change nickname, you have no permission.")
        ...
        return Response(":white_check_mark: Nickname has been changed.", delete_after=20)
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        try:
            ...
            raise exceptions.CommandError(":x: Unable to change avatar: %s" % e, expire_in=20)

        return Response(":white_check_mark: Avatar has been changed.", delete_after=20)


    async def cmd_disconnect(self, server):
        await self.disconnect_voice_client(server)
        return Response("Disconnecting.", delete_after=20)

    async def cmd_restart(self, channel):
        await self.safe_send_message(channel, ":arrows_counterclockwise: Restarting.")
        await self.disconnect_all_voice_clients()
        raise exceptions.RestartSignal

    async def cmd_shutdown(self, channel):
        await self.safe_send_message(channel, ":stop_button: Shutting down.")
        await self.disconnect_all_voice_clients()
    ...
    async def on_message(self, message):
        ...
        if message.author == self.user:
            self.safe_print("Ignoring commands from myself (%s)." % message.content)
            return
        ...
        if message.channel.is_private:
            if not (message.author.id == self.config.owner_id and command == 'joinserver'):
                await self.send_message(message.channel, ':octagonal_sign: You cannot use this bot in private messages.')
                return
        ...
        try:
            ...
            if message.author.id != self.config.owner_id:
                if user_permissions.command_whitelist and command not in user_permissions.command_whitelist:
                    raise exceptions.PermissionsError(
                        ":x: This command is not enabled for your group (%s)." % user_permissions.name,
                        expire_in=20)

                elif user_permissions.command_blacklist and command in user_permissions.command_blacklist:
                    raise exceptions.PermissionsError(
                        ":x: This command is disabled for your group (%s)." % user_permissions.name,
                        expire_in=20)
